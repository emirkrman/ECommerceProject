...
public class ProductController : BaseController
{
    ...
    public async Task<IActionResult> List(int? categoryId, string? search, string? sort = null, int page = 1)
    {
        ...
        var products = await query
            .Skip((page - 1) * ProductListPageSize)
    ...
    public async Task<IActionResult> Create(ProductFormViewModel model)
    {
        ...
        var entity = CreateProductEntity(model, await SaveImageAsync(model.ImageFile));
    ...
    public async Task<IActionResult> Edit(int id)
    {
        ...
        await SetCategoriesViewBagAsync(product.CategoryId);
    ...
    private static Product CreateProductEntity(ProductFormViewModel model, string? imageUrl)
    {
        return new Product
        {
            Name = model.Name,
    ...
    private static ProductFormViewModel MapToFormModel(Product product)
    {
    ...
    private static void ApplyFormValues(Product entity, ProductFormViewModel model)
    {
    ...
    {
        SetCategoriesViewBag(await GetActiveCategoriesAsync(), selectedCategoryId);
    }
    ...
    private void ValidateImage(IFormFile? imageFile)
    {
        ...
        {
            ModelState.AddModelError(nameof(ProductFormViewModel.ImageFile), "Lütfen JPG, PNG veya WEBP formatında bir görsel seçin.");
        }

        if (!AllowedImageContentTypes.Contains(imageFile.ContentType, StringComparer.OrdinalIgnoreCase))
        {
            ModelState.AddModelError(nameof(ProductFormViewModel.ImageFile), "Yüklenen dosya geçerli bir görsel olmalı.");
        }

        if (imageFile.Length > MaxImageSizeBytes)
        {
            ModelState.AddModelError(nameof(ProductFormViewModel.ImageFile), "Görsel boyutu 2 MB'dan büyük olamaz.");
        }
    }
